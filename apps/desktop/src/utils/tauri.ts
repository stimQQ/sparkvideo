 ...
 * 保存当前应用版本到设置中，用于更新检测
 */
async saveCurrentVersion() : Promise<null> {
    return await TAURI_INVOKE("save_current_version");
},
/**
 * 获取保存的上次版本号
 */
async getLastSavedVersion() : Promise<string | null> {
    return await TAURI_INVOKE("get_last_saved_version");
},
/**
 * 获取当前应用版本号
 */
async getCurrentAppVersion() : Promise<string> {
    return await TAURI_INVOKE("get_current_app_version");
},
...
async denoiseRecordingAudio(recordingPath: string) : Promise<null> {
    return await TAURI_INVOKE("denoise_recording_audio", { recordingPath });
}
}
...
export type ClickEffectConfiguration = { enabled?: boolean; color?: string; size?: number; duration?: number; opacity?: number }
...
export type CurrentRecording = { target: CurrentRecordingTarget }
...
export type CursorConfiguration = { hide?: boolean; hideWhenIdle?: boolean; hideWhenIdleDelay?: number; size: number; type: CursorType; animationStyle: CursorAnimationStyle; tension: number; mass: number; friction: number; raw?: boolean; motionBlur?: number; useSvg?: boolean; clickEffect?: ClickEffectConfiguration }
...
export type DenoiseProgress = { progress: number; status: string }
...
export type HotkeyAction = "startStudioRecording" | "stopRecording" | "restartRecording" | "openRecordingPicker" | "openRecordingPickerDisplay" | "openRecordingPickerWindow" | "openRecordingPickerArea" | "other"
...
export type RecordingMeta = (StudioRecordingMeta) & { platform?: Platform | null; pretty_name: string; sharing?: SharingMeta | null; upload?: UploadMeta | null }
export type RecordingMetaWithMetadata = ((StudioRecordingMeta) & { platform?: Platform | null; pretty_name: string; sharing?: SharingMeta | null; upload?: UploadMeta | null }) & { status: StudioRecordingStatus }
export type RecordingOptionsChanged = null
export type RecordingSettingsStore = { target: ScreenCaptureTarget | null; micName: string | null; cameraId: DeviceOrModelID | null; systemAudio: boolean; organizationId: string | null }
...
export type StartRecordingInputs = { capture_target: ScreenCaptureTarget; capture_system_audio?: boolean }
...
export type UploadMeta = { state: "SinglePartUpload"; video_id: string; recording_dir: string; file_path: string; screenshot_path: string } | { state: "Failed"; error: string } | { state: "Complete" }
