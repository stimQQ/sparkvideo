...
use cap_project::{
    ...
    StudioRecordingStatus, UploadMeta, VideoUploadInfo, XY, ZoomSegment,
};
use cap_recording::{
    feeds::{
        self,
...
use crate::upload::{build_video_meta, create_or_get_video};

#[allow(clippy::large_enum_variant)]
pub enum RecordingState {
    None,
    Pending { target: ScreenCaptureTarget },
...
impl App {
    pub fn set_pending_recording(&mut self, target: ScreenCaptureTarget) {
        self.recording_state = RecordingState::Pending { target };
...
async fn get_current_recording(
    ...
    let capture_target = match &state.recording_state {
        RecordingState::None => return Ok(JsonValue::new(&None)),
        RecordingState::Pending { target } => target,
        RecordingState::Active(inner) => inner.capture_target(),
    };
    ...
    Ok(JsonValue::new(&Some(CurrentRecording { target })))
}
...
async fn get_video_metadata(path: PathBuf) -> Result<VideoRecordingMetadata, String> {
    ...
    let meta = recording_meta.studio_meta();
...
async fn save_file_dialog(
    ...
    let file_name = file_name
        .strip_suffix(".sparkvideo")
        .or_else(|| file_name.strip_suffix(".cap"))
        .unwrap_or(&file_name)
        .to_string();
    println!("File name after removing .sparkvideo/.cap suffix: {file_name}");
...
#[specta::specta]
#[instrument(skip(app))]
async fn import_video_file(app: AppHandle, video_path: PathBuf) -> Result<PathBuf, String> {
    use cap_project::{MultipleSegment, MultipleSegments, VideoMeta};
    ...
    let video_stream = input
        ...
        .ok_or_else(|| "No video stream found in the file".to_string())?;

    let fps = {
        let rate = video_stream.avg_frame_rate();
        if rate.1 > 0 {
            (rate.0 as f64 / rate.1 as f64).round() as u32
        } else {
            30
        }
    };
    ...
    let project_name = format!("Imported {}", date_time);
    let project_dir = recordings_dir.join(&project_name);

    std::fs::create_dir_all(&project_dir)
        .map_err(|e| format!("Failed to create project directory: {e}"))?;

    let content_dir = project_dir.join("content").join("segments").join("segment-0");
    std::fs::create_dir_all(&content_dir)
        .map_err(|e| format!("Failed to create content directory: {e}"))?;

    let video_extension = video_path
        .extension()
        .and_then(|s| s.to_str())
        .unwrap_or("mp4");
    let dest_video_path = content_dir.join(format!("display.{}", video_extension));

    std::fs::copy(&video_path, &dest_video_path)
        .map_err(|e| format!("Failed to copy video file: {e}"))?;

    let relative_video_path = RelativePathBuf::from(format!(
        "content/segments/segment-0/display.{}",
        video_extension
    ));
...
#[specta::specta]
#[instrument(skip(app))]
fn save_current_version(app: AppHandle) -> Result<(), String> {
    let current_version = app.package_info().version.to_string();
    println!("Saving current version: {}", current_version);

    GeneralSettingsStore::update(&app, |s| {
        s.last_version = Some(current_version);
    })
...
#[specta::specta]
#[instrument(skip(app))]
fn get_last_saved_version(app: AppHandle) -> Result<Option<String>, String> {
    GeneralSettingsStore::get(&app)
        .map_err(|e| format!("Failed to get settings: {e}"))
        .map(|s| s.and_then(|s| s.last_version))
}
...
#[specta::specta]
#[instrument(skip(app))]
fn get_current_app_version(app: AppHandle) -> String {
...
async fn reset_camera_permissions(_app: AppHandle) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let bundle_id = "com.sparkvideo.desktop";
...
pub async fn run(recording_logging_handle: LoggingHandle, logs_dir: PathBuf) {
    ...
    let specta_builder = tauri_specta::Builder::new()
        .commands(tauri_specta::collect_commands![
            ...
            noise_reduction::denoise_recording_audio
        ])
        .events(tauri_specta::collect_events![
            ...
            noise_reduction::DenoiseProgress,
        ])
    ...
    let mut builder =
        tauri::Builder::default().plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
            ...
            let Some(cap_file) = args
                .iter()
                .find(|arg| arg.ends_with(".sparkvideo") || arg.ends_with(".cap"))
                .map(PathBuf::from)
            else {
                let app = app.clone();
                tokio::spawn(async move {
