...
use layers::{
    Background, BackgroundLayer, BlurLayer, CameraLayer, CaptionsLayer, ClickRingLayer,
    CursorLayer, DisplayLayer,
};
use specta::Type;
...
impl RendererLayers {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        Self {
            ...
            click_ring: ClickRingLayer::new(device),
    ...
    pub fn render(
        ...
        // Render click ring effects after cursor
        if uniforms.scene.should_render_screen() && self.click_ring.has_active_clicks() {
            let mut pass = render_pass!(session.current_texture_view(), wgpu::LoadOp::Load);
            self.click_ring.render(&mut pass, queue, uniforms);
        }
        ...
        if uniforms.project.captions.as_ref().map_or(false, |c| c.settings.enabled) {
            let mut pass = render_pass!(session.current_texture_view(), wgpu::LoadOp::Load);
...
async fn produce_frame(
    ...
    layers.render(&constants.device, &constants.queue, &mut encoder, session, &uniforms);
