...
# Desktop (apps/desktop) — package name is @sparkvideo/desktop
cd apps/desktop && pnpm dev      # Start SolidStart + Tauri dev (use this, not pnpm run dev:desktop)
...
- **`target_display_crop: Display not found`**: macOS CGDisplayID changed after sleep/wake or display reconnect. Fixed in `crates/recording/src/capture_pipeline.rs` — falls back to `Display::primary()` automatically.
...
### Known Decisions
- **Mono mic mixing**: Mono microphone samples are mixed directly at `gain × 1.0` into both L/R channels. The original `0.707` power-preserving factor was removed (`crates/audio/src/renderer.rs`) because it made recordings sound too quiet.
- **Default mic gain**: `mic_volume_db` defaults to `20.0` dB (`crates/project/src/configuration.rs`). Adjust if recordings are too loud for a particular microphone.
- **Gain slider range**: Editor mic/system audio sliders range from -30 dB to +20 dB (`apps/desktop/src/routes/editor/ConfigSidebar.tsx`).

## Release & Distribution

### macOS Code Signing + Notarization
Fully automated in `.github/workflows/publish.yml`. Every triggered build is freshly signed and notarized — users will not see "unidentified developer" warnings.

**To release**: GitHub → Actions → `publish` → `Run workflow`

Required GitHub Secrets (configure once):
| Secret | Description |
|--------|-------------|
| `APPLE_CERTIFICATE` | .p12 certificate base64 |
| `APPLE_CERTIFICATE_PASSWORD` | .p12 password |
| `APPLE_SIGNING_IDENTITY` | e.g. `Developer ID Application: xxx` |
| `APPLE_API_ISSUER` | App Store Connect API Issuer ID |
| `APPLE_API_KEY` | App Store Connect API Key ID |
| `APPLE_API_KEY_FILE` | .p8 file contents |
| `TAURI_SIGNING_PRIVATE_KEY` | Updater signing private key |

### Download Page
Already exists at `apps/web/app/(site)/download/` and `apps/web/app/(site)/download/[platform]/route.ts`.
Downloads redirect to GitHub Releases (`https://github.com/YOUR_ORG/YOUR_REPO/releases/latest/download/SparkVideo_*.dmg`).
Update `YOUR_GITHUB_ORG/YOUR_REPO_NAME` in `[platform]/route.ts` to match the actual repository.

Auto-update endpoint: `https://sparkvideo.cc/api/releases/tauri/{{current_version}}/{{target}}/{{arch}}` (configured in `tauri.prod.conf.json`). This API route must be implemented in `apps/web`, or disable the updater by setting `"active": false` in `tauri.prod.conf.json`.
